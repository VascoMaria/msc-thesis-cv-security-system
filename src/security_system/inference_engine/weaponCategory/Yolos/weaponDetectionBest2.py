...
def get_model_path():
    if hasattr(sys, "_MEIPASS"):
        ...
        return os.path.join(sys._MEIPASS, "models", "best2.pt")
    else:
        # Caminho normal para o script
        return os.path.join(os.path.dirname(__file__), "models", "best2.pt")
...
def detect_weapons_batch(frames):
    """
    Detecta armas em múltiplos frames usando o modelo best2.pt em batch.
    ...
    print(f"\n🔍 Tempo total de inferência para {len(frames)} frames (best2): {total_time:.3f} segundos\n")
...
async def detect_batch(files: List[UploadFile] = File(...)):
    """
    Recebe múltiplos frames e retorna as detecções em batch pelo modelo best2.pt.
    """
    try:
        frames = []
        for file in files:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inicia o Modelo de armas best2.pt")
    parser.add_argument('--port', type=int, help='Porta para o Modelo de armas best2.pt (ex: 8007)', default=None)
    args = parser.parse_args()

    if args.port:
        port = args.port
    else:
        port = int(input("Digite a porta para iniciar o Modelo de armas best2.pt (ex: 8007): "))

    print("Iniciando o Modelo de armas best2.pt ...")
